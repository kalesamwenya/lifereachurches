...
useEffect(() => {
    ...
    const url = `${window.location.origin}/podcast?ep=${encodeURIComponent(currentEp.id)}`;

    document.title = `${currentEp.title} | Life Reach Podcast`;
    ...
    // Basic SEO
    setMeta("description", currentEp.desc, "name");

    // Open Graph (Facebook, WhatsApp, LinkedIn)
    setMeta("og:title", currentEp.title);
    setMeta("og:description", currentEp.desc);
    setMeta("og:image", currentEp.image);
    setMeta("og:url", url);
    setMeta("og:type", "article");

    // Twitter
    setMeta("twitter:card", "summary_large_image", "name");
    setMeta("twitter:title", currentEp.title, "name");
    setMeta("twitter:description", currentEp.desc, "name");
    setMeta("twitter:image", currentEp.image, "name");
 ...
 const togglePlay = (e) => {
   ...
   const playEpisode = (ep) => {
    ...
    const shareEpisode = async (ep) => {
    if (!ep) return;

    const shareUrl = `${window.location.origin}/podcast?ep=${encodeURIComponent(ep.id)}`;
...
/>
    ...
    onClick={(e) => {
        e.stopPropagation();
        shareEpisode(currentEp);
    }}
    className="text-gray-400 hover:text-orange-500 transition-colors flex flex-col items-center gap-1"
>
